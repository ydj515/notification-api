...
async def send_kakao(request: Request, body: KakaoMsgBody):
    token = os.environ.get("KAKAO_KEY", "dOAap7zvH9ri6kj53SzApCTBy41AS6E*****************g") # 환경변수에 KAKAO_KEY가 설정되어있다면 가져오고 아니면 오른쪽의 키값을 세팅
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/x-www-form-urlencoded"}

    body = dict(object_type="text", text=body.msg, link=dict(web_url="https://www.naver.com", mobile_url="https://www.naver.dingrr.com"), button_title="확인")
...
def send_email(**kwargs):
    ...
    if mailing_list:
        try:
            yag = yagmail.SMTP({email_addr: "ydj515@aaaa.com"}, email_pw)
            ...
            for m_l in mailing_list:
                ...
                sleep(1)
                yag.send(m_l.email, 'send.', contents)
                last_email = m_l.email
            return True
        except Exception as e:
            print(e)
            print(last_email)
    print("발송 실패시 slack이나 메일로 알려야함")