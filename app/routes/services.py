...
async def send_kakao(request: Request, body: KakaoMsgBody):
    ...
    body = dict(object_type="text", text=body.msg, link=dict(web_url="https://www.naver.com", mobile_url="https://www.naver.com"), button_title="지금 확인")
...
def send_email(**kwargs):
    ...
    if mailing_list:
        try:
            yag = yagmail.SMTP({email_addr: "호로록"}, email_pw)
...
async def email_by_ses():
    # 한국말 보내려면
    # ?UTF-8?B?[base64인코딩값]?
    sender = "Ryan =?UTF-8?B?7Jyg64+Z7KeE?= <sender@d9.is>"
    # sender = "Ryan 라이언 <sender@d9.is>"
    recipient = ["ydj515.info@gmail.com"]
